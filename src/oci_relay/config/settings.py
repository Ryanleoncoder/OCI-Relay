...
class Settings:
    def __init__(self):
        ...
        self.telegram_allowed_ids = [
            int(x.strip()) for x in (get_env_or_none("TELEGRAM_ALLOWED_CHAT_IDS") or "").split(",")
            if x.strip()
        ]
        ...
        self.oci_use_instance_principal = get_env_bool("OCI_USE_INSTANCE_PRINCIPAL", True)
        ...
        self.alert_memory_warning_percent = get_env_int("ALERT_MEMORY_WARNING_PERCENT", 85)
        self.alert_memory_critical_percent = get_env_int("ALERT_MEMORY_CRITICAL_PERCENT", 95)

        self.alert_disk_warning_percent = get_env_int("ALERT_DISK_WARNING_PERCENT", 80)
        self.alert_disk_critical_percent = get_env_int("ALERT_DISK_CRITICAL_PERCENT", 95)

        self.alert_attack_failures = get_env_int("ALERT_ATTACK_FAILURES", 100)
        self.alert_attack_window_minutes = get_env_int("ALERT_ATTACK_WINDOW_MINUTES", 10)
